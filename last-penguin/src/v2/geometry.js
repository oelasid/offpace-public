...
export function makeToonGradientMap() {
  const data = new Uint8Array([72,72,72,255,128,128,128,255,198,198,198,255,255,255,255,255]);
...
export function makeBlobGeometry(size, options = {}) {
  const { widthSegments = 40, heightSegments = 28, belly = 0, topTaper = 0, bottomTaper = 0, flattenBottom = 0, frontBias = 0 } = options;
  ...
  for (let i = 0; i < pos.count; i += 1) {
    ...
    if (frontBias !== 0 && z > 0) z *= 1 + frontBias * (1 - Math.abs(yn));
    pos.setXYZ(i, x, y, z);
  }
...
export function makeWingGeometry(size) {
  const geo = makeBlobGeometry(size, { widthSegments: 34, heightSegments: 26, belly: 0.03, topTaper: -0.34, bottomTaper: -0.07, flattenBottom: 0.01 });
  const pos = geo.attributes.position;
  const h = size[1];
  for (let i = 0; i < pos.count; i += 1) {
...
export function makeRoundedBeakGeometry(size) {
  const geo = makeBlobGeometry(size, { widthSegments: 32, heightSegments: 20, frontBias: 0.22 });
  const pos = geo.attributes.position;
  const [w] = size;
  for (let i = 0; i < pos.count; i += 1) {
    let x = pos.getX(i), y = pos.getY(i), z = pos.getZ(i);
...
export function makeClosedEyeGeometry(width = 0.22, rise = 0.07, thickness = 0.022) {
  const curve = new THREE.QuadraticBezierCurve3(new THREE.Vector3(-width * 0.5,0,0), new THREE.Vector3(0,rise,0), new THREE.Vector3(width * 0.5,0,0));
  return new THREE.TubeGeometry(curve, 12, thickness, 8, false);
}

export function addOutline(mesh, color = 0x181922, factor = 1.028) {
